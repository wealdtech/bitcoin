...
class ScriptChunk
{
  ...
  public ScriptChunk(final boolean isOpCode, final byte[] data, final int startLocationInProgram)
  {
 ...
 */
public class OldScript implements Comparable<OldScript>
{
...
//    @Inject
//    private Script(final Network network)
//    {
...
//    }
     ...
     * @throws DataError
     */
    public OldScript(byte[] programBytes, int offset, int length) throws DataError {
    ...
    private byte[] getData(int len) throws DataError {
        if (len > this.program.length - this.cursor)
            throw new DataError("Failed read of " + len + " bytes");
        try {
            ...
            throw new DataError("Failed read of " + len + " bytes", e);
    ...
    private int readByte() throws DataError {
        try {
            return 0xFF & this.program[this.cursor++];
        } catch (ArrayIndexOutOfBoundsException e) {
            throw new DataError("Attempted to read outside of script boundaries");
        }
    }
     ...
     */
    private void parse(byte[] programBytes, int offset, int length) throws DataError {
        // TODO: this is inefficient
        this.program = new byte[length];
        System.arraycopy(programBytes, offset, this.program, 0, length);

        offset = 0;
        this.chunks = new ArrayList<ScriptChunk>(10);  // Arbitrary choice of initial size.
        this.cursor = offset;
        while (this.cursor < offset + length) {
            ...
            if (opcode >= 0 && opcode < OP_PUSHDATA1) {
                ...
                this.chunks.add(new ScriptChunk(false, getData(opcode), startLocationInProgram));  // opcode == len here.
            } else if (opcode == OP_PUSHDATA1) {
                int len = readByte();
                this.chunks.add(new ScriptChunk(false, getData(len), startLocationInProgram));
                ...
                this.chunks.add(new ScriptChunk(false, getData((int)len), startLocationInProgram));
            } else {
...
//            return false;
//        return this.chunks.get(1).equalsOpCode(OP_CHECKSIG) &&
//                !this.chunks.get(0).isOpCode && this.chunks.get(0).data.length > 1;
//    }
...
//        if (this.chunks.size() != 5) return false;
//        return this.chunks.get(0).equalsOpCode(OP_DUP) &&
//               this.chunks.get(1).equalsOpCode(OP_HASH160) &&
//               this.chunks.get(2).data.length == Address.LENGTH &&
//               this.chunks.get(3).equalsOpCode(OP_EQUALVERIFY) &&
//               this.chunks.get(4).equalsOpCode(OP_CHECKSIG);
//    }

    /**
     * If a program matches the standard template DUP HASH160 <pubkey hash> EQUALVERIFY CHECKSIG
     * then this function retrieves the third element, otherwise it throws a DataError.<p>
     *
     * This is useful for fetching the destination address of a transaction.
     */
//    public byte[] getPubKeyHash() throws DataError {
//        if (!isSentToAddress())
//            throw new DataError("Script not in the standard scriptPubKey form");
...
//    }
     ...
     *
     * @throws DataError if the script is none of the named forms.
     */
//    public byte[] getPubKey() throws DataError {
//        if (this.chunks.size() != 2) {
//            throw new DataError("Script not of right size, expecting 2 but got " + this.chunks.size());
//        }
//        if (this.chunks.get(0).data.length > 2 && this.chunks.get(1).data.length > 2) {
//            // If we have two large constants assume the input to a pay-to-address output.
//            return this.chunks.get(1).data;
//        } else if (this.chunks.get(1).data.length == 1 && this.chunks.get(1).equalsOpCode(OP_CHECKSIG) && this.chunks.get(0).data.length > 2) {
//            // A large constant followed by an OP_CHECKSIG is the key.
//            return this.chunks.get(0).data;
//        } else {
//            throw new DataError("Script did not match expected form: " + toString());
//        }
//    }
     ...
     */
//    public Address getFromAddress() throws DataError {
...
//    }
     ...
     * @throws DataError
     */
//    public Address getToAddress() throws DataError {
...
//    private static int getSigOpCount(List<ScriptChunk> chunks, boolean accurate) throws DataError {
//        int sigOps = 0;
...
//    }
     ...
     */
    private static int getOpNValue(int opcode) throws DataError {
        if (opcode == OP_0)
            return 0;
        if (opcode < OP_1 || opcode > OP_16) // This should absolutely never happen
            throw new DataError("getOpNValue called on non OP_N opcode");
     ...
     */
//    public static int getSigOpCount(byte[] program) throws DataError {
...
//    }
     ...
     */
//    public static long getP2SHSigOpCount(byte[] scriptSig) throws DataError {
//        Script script = new Script();
//        try {
...
//        return this.program.length == 23 &&
//               (this.program[0] & 0xff) == OP_HASH160 &&
//               (this.program[1] & 0xff) == 0x14 &&
//               (this.program[22] & 0xff) == OP_EQUAL;
//    }
...
//    }
    ...
    private static BigInteger castToBigInteger(byte[] chunk) throws DataError {
        if (chunk.length > 4)
            throw new DataError("Script attempted to use an integer larger than 4 bytes");
        return Utils.decodeMPI(Utils.reverseBytes(chunk), false);
    }

    private static void executeScript(Transaction txContainingThis, long index,
                                      OldScript script, LinkedList<byte[]> stack) throws DataError {
        int opCount = 0;
        ...
        for (ScriptChunk chunk : script.chunks) {
            ...
            if (!chunk.isOpCode) {
                if (chunk.data.length > 520)
                    throw new DataError("Attempted to push a data string larger than 520 bytes");
                ...
                if (opcode > OP_16) {
                    opCount++;
                    if (opCount > 201)
                        throw new DataError("More script operations than is allowed");
                }

                if (opcode == OP_VERIF || opcode == OP_VERNOTIF)
                    throw new DataError("Script included OP_VERIF or OP_VERNOTIF");

                if (opcode == OP_CAT || opcode == OP_SUBSTR || opcode == OP_LEFT || opcode == OP_RIGHT ||
                    opcode == OP_INVERT || opcode == OP_AND || opcode == OP_OR || opcode == OP_XOR ||
                    opcode == OP_2MUL || opcode == OP_2DIV || opcode == OP_MUL || opcode == OP_DIV ||
                    opcode == OP_MOD || opcode == OP_LSHIFT || opcode == OP_RSHIFT)
                    throw new DataError("Script included a disabled Script Op.");
                ...
                case OP_IF:
                    ...
                    if (stack.size() < 1)
                        throw new DataError("Attempted OP_IF on an empty stack");
                ...
                case OP_NOTIF:
                    ...
                    if (stack.size() < 1)
                        throw new DataError("Attempted OP_NOTIF on an empty stack");
                    ifStack.add(!castToBool(stack.pollLast()));
                    continue;
                case OP_ELSE:
                    if (ifStack.isEmpty())
                        throw new DataError("Attempted OP_ELSE without OP_IF/NOTIF");
                    ifStack.add(!ifStack.pollLast());
                    continue;
                case OP_ENDIF:
                    if (ifStack.isEmpty())
                        throw new DataError("Attempted OP_ENDIF without OP_IF/NOTIF");
                    ifStack.pollLast();
                    continue;
                }
                ...
                case OP_VERIFY:
                    if (stack.size() < 1)
                        throw new DataError("Attempted OP_VERIFY on an empty stack");
                    if (!castToBool(stack.pollLast()))
                        throw new DataError("OP_VERIFY failed");
                    break;
                case OP_RETURN:
                    throw new DataError("Script called OP_RETURN");
                case OP_TOALTSTACK:
                    if (stack.size() < 1)
                        throw new DataError("Attempted OP_TOALTSTACK on an empty stack");
                    altstack.add(stack.pollLast());
                    break;
                case OP_FROMALTSTACK:
                    if (altstack.size() < 1)
                        throw new DataError("Attempted OP_TOALTSTACK on an empty altstack");
                    stack.add(altstack.pollLast());
                    break;
                case OP_2DROP:
                    if (stack.size() < 2)
                        throw new DataError("Attempted OP_2DROP on a stack with size < 2");
                    stack.pollLast();
                    stack.pollLast();
                    break;
                case OP_2DUP:
                    if (stack.size() < 2)
                        throw new DataError("Attempted OP_2DUP on a stack with size < 2");
                ...
                case OP_3DUP:
                    if (stack.size() < 3)
                        throw new DataError("Attempted OP_3DUP on a stack with size < 3");
                ...
                case OP_2OVER:
                    if (stack.size() < 4)
                        throw new DataError("Attempted OP_2OVER on a stack with size < 4");
                ...
                case OP_2ROT:
                    if (stack.size() < 6)
                        throw new DataError("Attempted OP_2ROT on a stack with size < 6");
                ...
                case OP_2SWAP:
                    if (stack.size() < 4)
                        throw new DataError("Attempted OP_2SWAP on a stack with size < 4");
                ...
                case OP_IFDUP:
                    if (stack.size() < 1)
                        throw new DataError("Attempted OP_IFDUP on an empty stack");
                ...
                case OP_DROP:
                    if (stack.size() < 1)
                        throw new DataError("Attempted OP_DROP on an empty stack");
                    stack.pollLast();
                    break;
                case OP_DUP:
                    if (stack.size() < 1)
                        throw new DataError("Attempted OP_DUP on an empty stack");
                    stack.add(stack.getLast());
                    break;
                case OP_NIP:
                    if (stack.size() < 2)
                        throw new DataError("Attempted OP_NIP on a stack with size < 2");
                ...
                case OP_OVER:
                    if (stack.size() < 2)
                        throw new DataError("Attempted OP_OVER on a stack with size < 2");
                ...
                case OP_ROLL:
                    if (stack.size() < 1)
                        throw new DataError("Attempted OP_PICK/OP_ROLL on an empty stack");
                    long val = castToBigInteger(stack.pollLast()).longValue();
                    if (val < 0 || val >= stack.size())
                        throw new DataError("OP_PICK/OP_ROLL attempted to get data deeper than stack size");
                ...
                case OP_ROT:
                    if (stack.size() < 3)
                        throw new DataError("Attempted OP_ROT on a stack with size < 3");
                ...
                case OP_TUCK:
                    if (stack.size() < 2)
                        throw new DataError("Attempted OP_SWAP on a stack with size < 2");
                ...
                case OP_LEFT:
                case OP_RIGHT:
                    throw new DataError("Attempted to use disabled Script Op.");
                case OP_SIZE:
                    if (stack.size() < 1)
                        throw new DataError("Attempted OP_SIZE on an empty stack");
                ...
                case OP_EQUAL:
                    if (stack.size() < 2)
                        throw new DataError("Attempted OP_EQUALVERIFY on a stack with size < 2");
                ...
                case OP_EQUALVERIFY:
                    ...
                    if (!Arrays.equals(stack.pollLast(), stack.pollLast()))
                        throw new DataError("OP_EQUALVERIFY: non-equal data");
                    break;
                case OP_1ADD:
                case OP_1SUB:
                case OP_NEGATE:
                ...
                case OP_0NOTEQUAL:
                    if (stack.size() < 1)
                        throw new DataError("Attempted a numeric op on an empty stack");
                ...
                case OP_MAX:
                    if (stack.size() < 2)
                        throw new DataError("Attempted a numeric op on a stack with size < 2");
                ...
                case OP_NUMEQUALVERIFY:
                    if (stack.size() < 2)
                        throw new DataError("Attempted OP_NUMEQUALVERIFY on a stack with size < 2");
                    ...
                    if (!OPNUMEQUALVERIFYnum1.equals(OPNUMEQUALVERIFYnum2))
                        throw new DataError("OP_NUMEQUALVERIFY failed");
                    break;
                case OP_WITHIN:
                    if (stack.size() < 3)
                        throw new DataError("Attempted OP_WITHIN on a stack with size < 3");
                ...
                case OP_RIPEMD160:
                    if (stack.size() < 1)
                        throw new DataError("Attempted OP_RIPEMD160 on an empty stack");
                ...
                case OP_SHA1:
                    if (stack.size() < 1)
                        throw new DataError("Attempted OP_SHA1 on an empty stack");
                    try {
                ...
                case OP_SHA256:
                    if (stack.size() < 1)
                        throw new DataError("Attempted OP_SHA256 on an empty stack");
                    try {
                ...
                case OP_HASH160:
                    if (stack.size() < 1)
                        throw new DataError("Attempted OP_HASH160 on an empty stack");
                ...
                case OP_CHECKSIGVERIFY:
                    if (stack.size() < 2)
                        throw new DataError("Attempted OP_CHECKSIG(VERIFY) on a stack with size < 2");
                    ...
                    if (opcode == OP_CHECKSIG)
                        ...
                        if (!CHECKSIGsigValid)
                            throw new DataError("Script failed OP_CHECKSIGVERIFY");
                    break;
                case OP_CHECKMULTISIG:
                case OP_CHECKMULTISIGVERIFY:
                    if (stack.size() < 2)
                        throw new DataError("Attempted OP_CHECKMULTISIG(VERIFY) on a stack with size < 2");
                    ...
                    if (CHECKMULTISIGpubKeyCount < 0 || CHECKMULTISIGpubKeyCount > 20)
                        throw new DataError("OP_CHECKMULTISIG(VERIFY) with pubkey count out of range");
                    opCount += CHECKMULTISIGpubKeyCount;
                    if (opCount > 201)
                        throw new DataError("Total op count > 201 during OP_CHECKMULTISIG(VERIFY)");
                    if (stack.size() < CHECKMULTISIGpubKeyCount + 1)
                        throw new DataError("Attempted OP_CHECKMULTISIG(VERIFY) on a stack with size < num_of_pubkeys + 2");
                    ...
                    if (CHECKMULTISIGsigCount < 0 || CHECKMULTISIGsigCount > CHECKMULTISIGpubKeyCount)
                        throw new DataError("OP_CHECKMULTISIG(VERIFY) with sig count out of range");
                    if (stack.size() < CHECKMULTISIGsigCount + 1)
                        throw new DataError("Attempted OP_CHECKMULTISIG(VERIFY) on a stack with size < num_of_pubkeys + num_of_signatures + 3");
                    ...
                    if (opcode == OP_CHECKMULTISIG)
                        ...
                        if (!CHECKMULTISIGValid)
                            throw new DataError("Script failed OP_CHECKMULTISIGVERIFY");
                    break;
                case OP_NOP1:
                case OP_NOP2:
                case OP_NOP3:
                ...
                default:
                    throw new DataError("Script used a reserved Op Code");
                }
            }

            if (stack.size() + altstack.size() > 1000 || stack.size() + altstack.size() < 0)
                throw new DataError("Stack size exceeded range");
        }

        if (!ifStack.isEmpty())
            throw new DataError("OP_IF/OP_NOTIF without OP_ENDIF");
    }
     ...
     */
    public void correctlySpends(Transaction txContainingThis, long scriptSigIndex, OldScript scriptPubKey,
                                boolean enforceP2SH) throws DataError {
        if (this.program.length > 10000 || scriptPubKey.program.length > 10000)
            throw new DataError("Script larger than 10,000 bytes");
        ...
        if (stack.size() == 0)
            throw new DataError("Stack empty at end of script execution.");

        if (!castToBool(stack.pollLast()))
            throw new DataError("Script resulted in a non-true stack");
        ...
        if (enforceP2SH && scriptPubKey.isPayToScriptHash()) {
            for (ScriptChunk chunk : this.chunks)
                if (chunk.isOpCode && (chunk.data[0] & 0xff) > OP_16)
                    throw new DataError("Attempted to spend a P2SH scriptPubKey with a script that contained script ops");

            byte[] scriptPubKeyBytes = p2shStack.pollLast();
            OldScript scriptPubKeyP2SH = new OldScript(scriptPubKeyBytes, 0, scriptPubKeyBytes.length);

            executeScript(txContainingThis, scriptSigIndex, scriptPubKeyP2SH, p2shStack);

            if (p2shStack.size() == 0)
                throw new DataError("P2SH stack empty at end of script execution.");

            if (!castToBool(p2shStack.pollLast()))
                throw new DataError("P2SH script execution resulted in a non-true stack");
        }
    }
...
//  }
  ...
  @Override
  public int compareTo(final OldScript that)
  {
